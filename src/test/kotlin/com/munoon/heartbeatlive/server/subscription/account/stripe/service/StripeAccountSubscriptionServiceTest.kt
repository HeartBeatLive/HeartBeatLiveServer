...
internal class StripeAccountSubscriptionServiceTest : AbstractTest() {
    ...
    @MockkBean
    private lateinit var stripeAccountService: StripeAccountService
    ...
    fun createSubscription() {
        coEvery { stripeAccountService.getRequiredStripeAccount(any()) } returns StripeAccount(
            id = "user1", stripeAccountId = "stripeCustomer1")
        ...
        coVerify(exactly = 1) {
            client.createSubscription(matchSubscriptionCreateParams(expectedSubscriptionParams), matchUUID())
        }
        coVerify(exactly = 1) { stripeAccountService.getRequiredStripeAccount(user) }
    }
    ...
    fun saveFailedRecurringCharge() {
        ...
        coVerify(exactly = 2) {
            client.getInvoice("stripeInvoice1", matchUUID(), "payment_intent")
        }
    }
    ...
    private companion object {
        fun MockKMatcherScope.matchSubscriptionCreateParams(expectedSubscription: SubscriptionCreateParams) =
            match<SubscriptionCreateParams> {
                assertThat(it).usingRecursiveComparison().isEqualTo(expectedSubscription)
                true
            }

        fun MockKMatcherScope.matchSubscriptionUpdateParams(expectedSubscription: SubscriptionUpdateParams) =
            match<SubscriptionUpdateParams> {
                assertThat(it).usingRecursiveComparison().isEqualTo(expectedSubscription)
                true
            }
